...
def create_or_restore_session():
    ...
    previous_session_id = input("Enter the session_id of an already existing session to continue working with it (leave empty if you want to start a new session): ")
...
if __name__ == "__main__":
    ...
    git_repo_callback = GitLocalRepoCallbackHandler(username=username, session_path=session_path)

    memory = YeagerMemory(username, session_id, session_path)

    curate_memory = CurateNStoreMemory(memory=memory, session_id=session_id)

    # instantiate the YeagerKit
    tckit = YeagerKit()
    tckit.register_tool(CreateToolSourceRun(api_wrapper=CreateToolSourceAPIWrapper(session_path=session_path)))
